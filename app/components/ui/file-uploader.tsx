...
export default function FileUploader({
  ...
  const defaultCheckExtension = (extension: string) => {
    if (allowedExtensions && !allowedExtensions.includes(extension)) {
      return `Tipo de archivo inválido. Seleccione uno de estos formatos: ${allowedExtensions!.join(
        ",",
      )}`;
    }
    return null;
  ...
  const handleUpload = async (file: File) => {
    ...
    if (isFileSizeExceeded(file)) {
      return onFileUploadError(
        `Tamaño de archivo excedido. El Limite es ${fileSizeLimit / 1024 / 1024} MB`,
      );
    }
